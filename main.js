...
//
// The board is four SIBLING zones (Opponent / Hunting Grounds /
// Player / Hand) inside a CSS Grid with fixed-height rows, so no
// zone can steal height from another. See style.css #screen-game.
// ═══════════════════════════════════════════════════════════════
...
function renderStatusList() {
  ...
  if (!bar) {
    bar = document.createElement('div');
    bar.id = 'status-list';
    el('screen-game').insertBefore(bar, el('phase-bar').nextSibling);
  }
...
function renderPassBar() {
  ...
  if (!bar) {
    bar = document.createElement('div');
    bar.id = 'pass-bar';
    el('screen-game').insertBefore(bar, el('status-list').nextSibling);
  }
  bar.innerHTML = `
