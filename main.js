...
function renderActionBar() {
  ...
  // Confirm button for a selected-but-not-yet-played combat hand card.
  // Rendered INSIDE the action bar (not a separate floating element)
  // so it's guaranteed to use the exact same fixed-position container
  // that's already confirmed working, rather than its own positioning.
  if (selectedHandCard) {
    const confirmBtn = document.createElement('button');
    confirmBtn.className = 'action-btn action-btn-confirm';
    confirmBtn.textContent = `▶ Play ${selectedHandCard.name}`;
    confirmBtn.onclick = () => {
