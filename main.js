...
function openFullCard(card, startCrinos, isDual, onClose) {
  ...
  // Close strip at very top — always tappable
  ...
  // Inner card (stops propagation so tapping card doesn't close)
  const inner = document.createElement('div');
  inner.className = 'fullcard-inner';
  inner.addEventListener('click', e => e.stopPropagation());
  overlay.appendChild(inner);

  // Tap the overlay background (outside inner) to close
  ...
  cardImg.onerror = function() {
    this.onerror = null;
    this.src = `https://placehold.co/400x560/1a1a1a/ff4444?text=${encodeURIComponent(name)}`;
  };

  if (isDual) {
    const flipL = document.createElement('button');
    flipL.className = 'fullcard-flip-btn fullcard-flip-left';
    flipL.textContent = '⇄';
    flipL.title = 'Flip card';
    flipL.addEventListener('click', e => { e.stopPropagation(); doFlip(); });
    imgWrap.appendChild(flipL);
  }

  imgWrap.appendChild(cardImg);

  if (isDual) {
    const flipR = document.createElement('button');
    flipR.className = 'fullcard-flip-btn fullcard-flip-right';
    flipR.textContent = '⇄';
    flipR.title = 'Flip card';
    flipR.addEventListener('click', e => { e.stopPropagation(); doFlip(); });
    imgWrap.appendChild(flipR);
  }
  ...
  // Stats block — updated on flip
  const statsBlock = document.createElement('div');
  statsBlock.className = 'full-stats';
  // Insert stats before card text
  const textEl = info.querySelector('.fullcard-text') ||
                 info.querySelector('.fullcard-errata');
  if (textEl) info.insertBefore(statsBlock, textEl);
  else info.appendChild(statsBlock);
  ...
  function updateView() {
    // Image
    cardImg.src = getImagePath(card, isCrinos);
