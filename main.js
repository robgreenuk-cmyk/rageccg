...
// Combat hand: card the player has tapped but not yet confirmed playing.
// Pure UI state, not part of GameState — cleared on play, on deselect,
// or automatically if the round moves on before it's confirmed.
...
function renderCombatHandRow() {
  ...
  // Safety: if the round moved on (or ended) before a selection was
  // confirmed, don't leave a stale card glowing as "selected".
  if (selectedHandCard && !playableCards.includes(selectedHandCard)) {
  ...
  hand.forEach(inst => {
    ...
    const card = buildCard(inst.def, {
      mode:     'board',
      instance: inst,
      // Tapping a playable card SELECTS it (name/stats area, same
      // tap target as ever — the image still always opens the normal
      // full-screen viewer, unchanged everywhere in the app). Playing
      // it is a separate, explicit confirm via the floating button
      // below, not an instant action from a single tap.
      onSelect: canPlay ? () => {
        selectedHandCard = (selectedHandCard === inst) ? null : inst;
        renderCombatHandRow();
        renderPlayConfirmBar();
      } : null,
    });
    card.classList.toggle('is-playable', canPlay);
    card.classList.toggle('is-selected', selectedHandCard === inst);
    row.appendChild(card);
  });
}

// Floating confirm button — appears only once a combat hand card has
// been selected (tapped, not yet played). This is the actual "commit"
// step; selecting a card never plays it by itself.
function renderPlayConfirmBar() {
  let bar = el('play-confirm-bar');
  ...
  bar.innerHTML = `<button class="play-confirm-btn">▶ Play ${selectedHandCard.name}</button>`;
  bar.querySelector('.play-confirm-btn').addEventListener('click', () => {
    const card = selectedHandCard;
    selectedHandCard = null;
    handlePlayerAction({ type: 'PLAY_COMBAT_CARD', card });
  });

  // Position directly above the actual rendered action bar. Computed
  // fresh via getBoundingClientRect rather than a CSS var, since this
  // bar lives on document.body (a sibling of #screen-game, not a
  // descendant) and can't inherit a var set on #screen-game.
  const actionBarEl = el('action-bar');
  const actionBarHeight = actionBarEl ? actionBarEl.getBoundingClientRect().height : 0;
  bar.style.bottom = (actionBarHeight + 14) + 'px';
}
  ...
  function render() {
    ...
    wrap.innerHTML = `
      <div class="card-img-wrap">
        ...
        ${dmg > 0 ? `<div class="wound-badge" title="${dmg} damage">−${dmg}</div>` : ''}
      </div>
    ...
    // The wound badge is a second, more obvious entry point to the
    // same swipeable viewer.
