...
function declareAttack(attackerWho, attacker, target) {
  ...
  // yet — that's Phase 4. targetInst is hardcoded to the opposing
  // alpha here since real target *assignment* (Phase 2, rule 6.7)
  // only matters once a side can have more than one participant.
  const attackerParticipants = [
    { inst: attacker, ownerWho: attackerWho, role: 'alpha',
      card: null, feintCard: null, feintDecided: false, targetInst: target },
  ];
  const defenderParticipants = [
    { inst: target, ownerWho: defenderWho, role: 'alpha',
      card: null, feintCard: null, feintDecided: false, targetInst: attacker },
  ];
  ...
  GameState.combat = {
    ...
    get attacker()  { return this.attackerParticipants.find(p => p.role === 'alpha').inst; },
    get defender()  { return this.defenderParticipants.find(p => p.role === 'alpha').inst; },

    get attackerCard()  { return this.attackerParticipants.find(p => p.role === 'alpha').card; },
    set attackerCard(v) { this.attackerParticipants.find(p => p.role === 'alpha').card = v; },
    get defenderCard()  { return this.defenderParticipants.find(p => p.role === 'alpha').card; },
    set defenderCard(v) { this.defenderParticipants.find(p => p.role === 'alpha').card = v; },
    ...
    get attackerFeintCard()  { return this.attackerParticipants.find(p => p.role === 'alpha').feintCard; },
    set attackerFeintCard(v) { this.attackerParticipants.find(p => p.role === 'alpha').feintCard = v; },
    get defenderFeintCard()  { return this.defenderParticipants.find(p => p.role === 'alpha').feintCard; },
    set defenderFeintCard(v) { this.defenderParticipants.find(p => p.role === 'alpha').feintCard = v; },

    get attackerFeintDecided()  { return this.attackerParticipants.find(p => p.role === 'alpha').feintDecided; },
    set attackerFeintDecided(v) { this.attackerParticipants.find(p => p.role === 'alpha').feintDecided = v; },
    get defenderFeintDecided()  { return this.defenderParticipants.find(p => p.role === 'alpha').feintDecided; },
    set defenderFeintDecided(v) { this.defenderParticipants.find(p => p.role === 'alpha').feintDecided = v; },
