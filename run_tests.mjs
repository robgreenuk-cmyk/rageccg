...
// Rule 6.6.1: whenever 2+ plays from the same side land on the same
// target in the same tier, combat pauses at 'damageOrder' for that
// side's controller to pick the resolution order. Most tests don't
// care WHICH card wins the tie (only that damage totals come out
// right), so this just picks whatever's offered first, repeatedly,
// until the pause clears.
function resolveAnyDamageOrder() {
  while (GameState.combat && GameState.combat.step === 'damageOrder') {
    const opts = getCombatRoundActions('player').some(a => a.type === 'CHOOSE_DAMAGE_ORDER')
      ? getCombatRoundActions('player') : getCombatRoundActions('opponent');
    const who = getCombatRoundActions('player').some(a => a.type === 'CHOOSE_DAMAGE_ORDER') ? 'player' : 'opponent';
    performCombatAction(who, opts[0]);
  }
}
...
// ═══ TEST 52: Phase 4 — Hunting Party (declaration step, Renown-capped pack-join) ═══
console.log('\n[52] Phase 4: Hunting Party — declaration-step pack attack with a Renown cap');

const huntingParty = byName('Hunting Party');

initGame(
  { characters: [tracerOppChar, timRowantree, passer, fillerChar], sept: [],
    combat: [huntingParty, glancingBlow, glancingBlow, glancingBlow] }, // Allamande (Renown 6) + pack
  { characters: [tracerPlayerChar], sept: [], combat: [] },             // Cernonous — defender alpha
  20
);

enterPhase('combat');
performAction('player', { type: 'SELECT_ALPHA', card: GameState.player.pack[0] }); // Allamande
const declare52 = getLegalActions('player').filter(a => a.type === 'DECLARE_ATTACK');
declareAttack('player', declare52[0].attacker, declare52[0].target);

check('combat paused at declaration for the attacker — Hunting Party is in hand',
  GameState.combat.step === 'declaration' && GameState.combat.stepWaitingOn[0] === 'player');

const hp52 = getCombatRoundActions('player').find(a => a.type === 'PLAY_PACK_ACTION' && a.card.name === 'Hunting Party').card;
performCombatAction('player', { type: 'PLAY_PACK_ACTION', card: hp52 });

check('Hunting Party pulled from hand and marked in-play, not discarded yet',
  hp52.zone === 'packActionInPlay' && !GameState.player.combatHand.includes(hp52));

// Renown budget: 15 total cap - Allamande's own 6 = 9 remaining.
let opts52 = getCombatRoundActions('player');
check('Carla Grimsson (Renown 7) is offered — fits under the initial 9-Renown remaining budget',
  opts52.some(a => a.type === 'JOIN_PACK_MEMBER' && a.inst.name === 'Carla Grimsson'));

// Bring in Tim Rowantree (Renown 3) — remaining cap 9 -> 6.
performCombatAction('player', {
  type: 'JOIN_PACK_MEMBER',
  inst: opts52.find(a => a.type === 'JOIN_PACK_MEMBER' && a.inst.name === 'Tim Rowantree').inst,
});

// Bring in Passer (Renown 1) — remaining cap 6 -> 5.
opts52 = getCombatRoundActions('player');
performCombatAction('player', {
  type: 'JOIN_PACK_MEMBER',
  inst: opts52.find(a => a.type === 'JOIN_PACK_MEMBER' && a.inst.name === 'Passer').inst,
});

opts52 = getCombatRoundActions('player');
check('Carla Grimsson (Renown 7) correctly excluded once she would exceed the remaining cap (7 > 5 left)',
  !opts52.some(a => a.type === 'JOIN_PACK_MEMBER' && a.inst.name === 'Carla Grimsson'));
check('a "done selecting" option is offered', opts52.some(a => a.type === 'FINISH_PACK_JOIN'));

performCombatAction('player', { type: 'FINISH_PACK_JOIN' });

check('both selected members joined attackerParticipants as packmembers',
  GameState.combat.attackerParticipants.length === 3
  && GameState.combat.attackerParticipants.some(p => p.inst.name === 'Tim Rowantree' && p.role === 'packmember')
  && GameState.combat.attackerParticipants.some(p => p.inst.name === 'Passer' && p.role === 'packmember'));
check('drew 1 bonus combat card per additional member (2 joined = 2 bonus cards recorded)',
  GameState.combat.packActions.find(pa => pa.card === hp52).bonusCards === 2);

// Nothing else eligible anywhere else — should cascade straight through
// preCombat/beginCombat to playCard off a single pass.
performCombatAction('player', { type: 'PASS_STEP' });
check('advanced all the way to playCard in one pass — nothing else pending', GameState.combat.step === 'playCard');

// Every attacker participant plays a card; defender passes throughout.
const allamande52 = GameState.combat.attackerParticipants.find(p => p.role === 'alpha').inst;
const cernonous52 = GameState.combat.defender;
for (let i = 0; i < 3; i++) {
  const pick = getCombatRoundActions('player').find(a => a.type === 'PLAY_COMBAT_CARD' && a.card.name === 'Glancing Blow');
  performCombatAction('player', { type: 'PLAY_COMBAT_CARD', card: pick.card });
}
performCombatAction('opponent', { type: 'PASS_COMBAT_CARD' });

check('targeting auto-resolved (defender has only 1 participant) straight to reveal', GameState.combat.step === 'reveal');
performCombatAction('player', { type: 'CONTINUE_REVEAL' });
resolveAnyDamageOrder();

check('all 3 attacker participants\' cards landed on Cernonous (3 damage)', totalDamage(cernonous52) === 3);

performCombatAction('player', { type: 'WITHDRAW' });
check('Hunting Party finally discarded for real now that combat has ended',
  hp52.zone === 'combatDiscard' && GameState.player.combatDiscard.includes(hp52));

// ═══ TEST 53: Phase 4 — Bum Rush (beginCombat step, whole pack, ONE round only) ═══
console.log('\n[53] Phase 4: Bum Rush — whole pack joins for one round, then auto-escapes');

const bumRush = byName('Bum Rush');

initGame(
  { characters: [tracerOppChar, timRowantree, passer], sept: [],
    combat: [bumRush, glancingBlow, glancingBlow, glancingBlow] }, // Allamande + 2 more pack members
  { characters: [tracerPlayerChar], sept: [], combat: [glancingBlow] },
  20
);

enterPhase('combat');
performAction('player', { type: 'SELECT_ALPHA', card: GameState.player.pack[0] }); // Allamande
const declare53 = getLegalActions('player').filter(a => a.type === 'DECLARE_ATTACK');
declareAttack('player', declare53[0].attacker, declare53[0].target);

check('Bum Rush is NOT eligible at declaration (beginOrBetween only) — cascades straight to beginCombat',
  GameState.combat.step === 'beginCombat' && GameState.combat.stepWaitingOn[0] === 'player');

const br53 = getCombatRoundActions('player').find(a => a.type === 'PLAY_PACK_ACTION' && a.card.name === 'Bum Rush').card;
performCombatAction('player', { type: 'PLAY_PACK_ACTION', card: br53 });

check('the whole remaining pack joined immediately — no selection step for wholePack mode',
  GameState.combat.attackerParticipants.length === 3
  && GameState.combat.attackerParticipants.some(p => p.inst.name === 'Tim Rowantree')
  && GameState.combat.attackerParticipants.some(p => p.inst.name === 'Passer'));
check('no bonus cards drawn for Bum Rush (unlike the other three join cards)',
  GameState.combat.packActions.find(pa => pa.card === br53).bonusCards === 0);

performCombatAction('player', { type: 'PASS_STEP' });
check('advanced to playCard', GameState.combat.step === 'playCard');

const cernonous53 = GameState.combat.defender;
for (let i = 0; i < 3; i++) {
  const pick = getCombatRoundActions('player').find(a => a.type === 'PLAY_COMBAT_CARD' && a.card.name === 'Glancing Blow');
  performCombatAction('player', { type: 'PLAY_COMBAT_CARD', card: pick.card });
}
...
check('all 3 pack members\' cards landed this round (3 damage) before anyone escapes',
  totalDamage(cernonous53) === 3);
check('still 3 attacker participants immediately after resolution — escape happens at round transition, not mid-round',
  GameState.combat.attackerParticipants.length === 3);

performCombatAction('player', { type: 'CONTINUE_COMBAT' });

check('Bum Rush\'s joiners escaped at the round transition — only the original alpha remains',
  GameState.combat.attackerParticipants.length === 1
  && GameState.combat.attackerParticipants[0].inst.name === 'Allamande');
check('Bum Rush itself finally discarded now that its one-round effect is over',
  br53.zone === 'combatDiscard' && GameState.player.combatDiscard.includes(br53));
check('escaped packmates are NOT in anyone\'s Victory Pile — this was an escape, not a death',
  !GameState.opponent.victoryPile.some(c => c.name === 'Tim Rowantree' || c.name === 'Passer'));
check('escaped packmates are still in their own pack — they just left combat, they did not die',
  GameState.player.pack.some(c => c.name === 'Tim Rowantree') && GameState.player.pack.some(c => c.name === 'Passer'));

// Close out cleanly: round 2 starts fresh with just the alpha; both pass.
check('round 2 started at playCard with only the alpha to act', GameState.combat.step === 'playCard');
performCombatAction('player', { type: 'PASS_COMBAT_CARD' });
performCombatAction('opponent', { type: 'PASS_COMBAT_CARD' });
performCombatAction('player', { type: 'CONTINUE_REVEAL' });
check('combat closed out normally afterward', GameState.combat === null);

// ═══ TEST 54: Phase 4 — Pack Defense (preCombat step, defender only) ═══
console.log('\n[54] Phase 4: Pack Defense — preCombat-step pack defense, defender-side only');

const packDefense = byName('Pack Defense');

initGame(
  { characters: [tracerOppChar], sept: [], combat: [glancingBlow] },                           // Allamande — attacker alpha
  { characters: [tracerPlayerChar, timRowantree], sept: [], combat: [packDefense, glancingBlow, glancingBlow] }, // Cernonous + Tim Rowantree
  20
);

enterPhase('combat');
performAction('player', { type: 'SELECT_ALPHA', card: GameState.player.pack[0] });
const declare54 = getLegalActions('player').filter(a => a.type === 'DECLARE_ATTACK');
declareAttack('player', declare54[0].attacker, declare54[0].target);

check('Pack Defense is NOT offered at declaration, even to the defender — cascades to preCombat',
  GameState.combat.step === 'preCombat' && GameState.combat.stepWaitingOn[0] === 'opponent');
check('the ATTACKER never sees Pack Defense as an option (defender-only card)',
  !getCombatRoundActions('player').some(a => a.type === 'PLAY_PACK_ACTION'));

const pd54 = getCombatRoundActions('opponent').find(a => a.type === 'PLAY_PACK_ACTION' && a.card.name === 'Pack Defense').card;
performCombatAction('opponent', { type: 'PLAY_PACK_ACTION', card: pd54 });

const joinOpt54 = getCombatRoundActions('opponent').find(a => a.type === 'JOIN_PACK_MEMBER' && a.inst.name === 'Tim Rowantree');
performCombatAction('opponent', { type: 'JOIN_PACK_MEMBER', inst: joinOpt54.inst });
performCombatAction('opponent', { type: 'FINISH_PACK_JOIN' });

check('Tim Rowantree joined the DEFENDER\'s side specifically', GameState.combat.defenderParticipants.length === 2
  && GameState.combat.defenderParticipants.some(p => p.inst.name === 'Tim Rowantree'));
check('1 bonus combat card drawn for the 1 additional member',
  GameState.combat.packActions.find(pa => pa.card === pd54).bonusCards === 1);

performCombatAction('opponent', { type: 'PASS_STEP' });
check('advanced to playCard', GameState.combat.step === 'playCard');

performCombatAction('player', { type: 'WITHDRAW' });

// ═══ TEST 55: Phase 4 — Surprise Ally (beginCombat/betweenRounds, attacker only, exactly 1 member) ═══
console.log('\n[55] Phase 4: Surprise Ally — attacker-only, exactly one member, no cap selection needed');

const surpriseAlly = byName('Surprise Ally');

initGame(
  { characters: [tracerOppChar, fillerChar, timRowantree], sept: [],
    combat: [surpriseAlly, glancingBlow, glancingBlow] }, // Allamande + 2 spare pack members
  { characters: [tracerPlayerChar], sept: [], combat: [] },
  20
);
...
enterPhase('combat');
performAction('player', { type: 'SELECT_ALPHA', card: GameState.player.pack[0] });
const declare55 = getLegalActions('player').filter(a => a.type === 'DECLARE_ATTACK');
declareAttack('player', declare55[0].attacker, declare55[0].target);

check('Surprise Ally not offered at declaration/preCombat — cascades to beginCombat',
  GameState.combat.step === 'beginCombat');

const sa55 = getCombatRoundActions('player').find(a => a.type === 'PLAY_PACK_ACTION' && a.card.name === 'Surprise Ally').card;
performCombatAction('player', { type: 'PLAY_PACK_ACTION', card: sa55 });

const joinOpts55 = getCombatRoundActions('player').filter(a => a.type === 'JOIN_PACK_MEMBER');
check('both spare pack members are offered as the one pick (fixedCount mode, no Renown filtering)',
  joinOpts55.length === 2);
performCombatAction('player', { type: 'JOIN_PACK_MEMBER', inst: joinOpts55[0].inst });

check('selection auto-finalized after exactly 1 pick — back to normal step actions, no FINISH needed',
  GameState.combat.packJoinSelection === null
  && GameState.combat.attackerParticipants.length === 2);
check('exactly 1 member joined, not both', GameState.combat.attackerParticipants.filter(p => p.role === 'packmember').length === 1);
check('1 bonus card drawn', GameState.combat.packActions.find(pa => pa.card === sa55).bonusCards === 1);
...
// ═══ TEST 56: Phase 4 — Shieldmate (declaration step, defender reacts, targeting INVERSION) ═══
console.log('\n[56] Phase 4: Shieldmate — joins reactively, then the DEFENDER chooses who absorbs incoming cards');

const shieldmate = byName('Shieldmate');

initGame(
  { characters: [tracerOppChar], sept: [], combat: [glancingBlow] },                              // Allamande — attacker alpha
  { characters: [tracerPlayerChar, timRowantree], sept: [], combat: [shieldmate, glancingBlow] },  // Cernonous — defender alpha, reacting
  20
);

enterPhase('combat');
performAction('player', { type: 'SELECT_ALPHA', card: GameState.player.pack[0] });
const declare56 = getLegalActions('player').filter(a => a.type === 'DECLARE_ATTACK');
declareAttack('player', declare56[0].attacker, declare56[0].target);

check('Shieldmate is offered to the DEFENDER at declaration — reacting to being attacked',
  GameState.combat.step === 'declaration' && GameState.combat.stepWaitingOn[0] === 'opponent');

const sm56 = getCombatRoundActions('opponent').find(a => a.type === 'PLAY_PACK_ACTION' && a.card.name === 'Shieldmate').card;
performCombatAction('opponent', { type: 'PLAY_PACK_ACTION', card: sm56 });

const joinOpt56 = getCombatRoundActions('opponent').find(a => a.type === 'JOIN_PACK_MEMBER' && a.inst.name === 'Tim Rowantree');
performCombatAction('opponent', { type: 'JOIN_PACK_MEMBER', inst: joinOpt56.inst });

check('Tim Rowantree joined as the shieldmate; selection auto-finalized (fixedCount=1)',
  GameState.combat.defenderParticipants.length === 2 && GameState.combat.packJoinSelection === null);
check('1 bonus card drawn', GameState.combat.packActions.find(pa => pa.card === sm56).bonusCards === 1);
check('a shieldmate group was registered pairing Cernonous with Tim Rowantree',
  GameState.combat.shieldmateGroups.length === 1
  && GameState.combat.shieldmateGroups[0].memberInstanceIds.length === 2);

performCombatAction('opponent', { type: 'PASS_STEP' });
check('advanced to playCard', GameState.combat.step === 'playCard');

const cernonous56 = GameState.combat.defenderParticipants.find(p => p.role === 'alpha').inst;
const timRowantree56 = GameState.combat.defenderParticipants.find(p => p.role === 'packmember').inst;

const gb56 = getCombatRoundActions('player')
  .find(a => a.type === 'PLAY_COMBAT_CARD' && a.card.name === 'Glancing Blow').card;
performCombatAction('player', { type: 'PLAY_COMBAT_CARD', card: gb56 });
const dgb56 = getCombatRoundActions('opponent')
  .find(a => a.type === 'PLAY_COMBAT_CARD' && a.card.name === 'Glancing Blow').card;
performCombatAction('opponent', { type: 'PLAY_COMBAT_CARD', card: dgb56 });
performCombatAction('opponent', { type: 'PASS_COMBAT_CARD' }); // Tim Rowantree's turn — nothing left to play, passes

// The attacker's card would normally require THE ATTACKER to choose
// between Cernonous/Tim Rowantree (rule 6.7's default, since the
// defending side has 2 participants) — Shieldmate inverts that.
check('the ATTACKER is NOT asked to choose — the decision routed to the defender instead',
  GameState.combat.step === 'targeting' && GameState.combat.targetingQueue.turn === 'opponent'
  && GameState.combat.targetingQueue.attackerQueue.length === 0
  && GameState.combat.targetingQueue.defenderQueue.length === 1);

const redirectOpts56 = getCombatRoundActions('opponent');
check('the defender is offered a free choice between Cernonous and the shieldmate',
  redirectOpts56.length === 2 && redirectOpts56.some(a => a.target === cernonous56) && redirectOpts56.some(a => a.target === timRowantree56));

// Deliberately redirect onto the SHIELDMATE, not the "obvious" original defender.
performCombatAction('opponent', { type: 'ASSIGN_TARGET', target: timRowantree56 });
performCombatAction('player', { type: 'CONTINUE_REVEAL' });

check("Allamande's card landed on the shieldmate (the chosen target), not Cernonous",
  timRowantree56.damageCards.includes(gb56) && !cernonous56.damageCards.includes(gb56));
check("Cernonous's own outgoing card still auto-targeted the lone attacker normally — shieldmate only affects INCOMING cards",
  cernonous56.damageCards.length === 0);

performCombatAction('player', { type: 'WITHDRAW' });

// ═══ TEST 57: Phase 4 — Taking the Death Blow (reactive redirect of the fatal card) ═══
console.log('\n[57] Phase 4: Taking the Death Blow — redirects the specific fatal card to a substitute, who becomes alpha');

const takingTheDeathBlow = byName('Taking the Death Blow');
initGame(
  { characters: [passer, fillerChar], sept: [], combat: [takingTheDeathBlow, glancingBlow] }, // Passer (Health 2, about to die) + Carla Grimsson (substitute, Health 3)
  { characters: [tracerOppChar],      sept: [], combat: [headWound] },                        // Allamande — attacker, Rage 6; Head Wound is Rage 5/Damage 2 — kills Passer but not the substitute once redirected
  20
);

enterPhase('combat');
performAction('player', { type: 'SELECT_ALPHA', card: GameState.player.pack[0] });   // Passer becomes the defender's alpha
performAction('opponent', { type: 'SELECT_ALPHA', card: GameState.opponent.pack[0] }); // Allamande attacks
const declare57 = getLegalActions('opponent').filter(a => a.type === 'DECLARE_ATTACK');
declareAttack('opponent', declare57[0].attacker, declare57[0].target); // opponent is attacker, player is defender this time

const passer57 = GameState.combat.defender; // Passer, about to take a mortal wound
const carla57 = GameState.player.pack.find(c => c.name === 'Carla Grimsson');

check('combat cascades straight to playCard — Taking the Death Blow only matters after damage, not before',
  GameState.combat.step === 'playCard');

const hw57 = getCombatRoundActions('opponent')
  .find(a => a.type === 'PLAY_COMBAT_CARD' && a.card.name === 'Head Wound').card;
performCombatAction('opponent', { type: 'PLAY_COMBAT_CARD', card: hw57 });
performCombatAction('player', { type: 'PASS_COMBAT_CARD' });
performCombatAction('opponent', { type: 'CONTINUE_REVEAL' });

check('Passer received the mortal wound (2 damage vs Health 2) — combat paused instead of finalizing the kill',
  GameState.combat.step === 'deathBlow' && isDead(passer57));
check('combat is still open — checkCombatDeaths has NOT run yet', GameState.combat !== null && GameState.opponent.victoryPile.length === 0);

const deathBlowOpts57 = getCombatRoundActions('player');
check('offered the option to save Passer with the eligible substitute (Carla Grimsson)',
  deathBlowOpts57.some(a => a.type === 'PLAY_DEATH_BLOW' && a.substituteInst === carla57)
  && deathBlowOpts57.some(a => a.type === 'PASS_STEP'));

const ttdb57 = deathBlowOpts57.find(a => a.type === 'PLAY_DEATH_BLOW').card;
performCombatAction('player', {
  type: 'PLAY_DEATH_BLOW', card: ttdb57, dyingInst: passer57, substituteInst: carla57,
});

check('Passer survived — the fatal card was redirected away, no other wounds changed', !isDead(passer57) && passer57.damageCards.length === 0);
check('Carla Grimsson now carries the redirected fatal card, and survives it (2 damage vs Health 3)', carla57.damageCards.includes(hw57) && !isDead(carla57));
check("Passer escaped combat (no 'cannot escape' flag exists anywhere yet, so escape always succeeds)",
  !GameState.combat.defenderParticipants.some(p => p.inst === passer57));
check('Carla Grimsson took over as the defending participant', GameState.combat.defenderParticipants.some(p => p.inst === carla57 && p.role === 'alpha'));
check('Carla Grimsson is now the player\'s alpha for the remainder of the Combat Phase', GameState.player.alpha === carla57);
check('neither creature ended up in a Victory Pile — this was a save, not a kill', GameState.opponent.victoryPile.length === 0);

check('combat proceeded normally afterward, not cut short', GameState.combat.step === 'withdrawal');
performCombatAction('opponent', { type: 'WITHDRAW' });

// ═══ TEST 58: Phase 4 — blow order genuinely changes kill/VP credit (rule 6.6.1) ═══
console.log('\n[58] Phase 4: choosing blow order changes which specific card gets the kill — and the VP that comes with it');

initGame(
  { characters: [tracerOppChar, fillerChar], sept: [], combat: [glancingBlow, tellingBlow] }, // Allamande + Carla Grimsson (Rage 3)
  { characters: [passer], sept: [], combat: [] }, // Passer — Health 2, single-form
  20
);

enterPhase('combat');
performAction('player', { type: 'SELECT_ALPHA', card: GameState.player.pack[0] });
const declare58 = getLegalActions('player').filter(a => a.type === 'DECLARE_ATTACK');
declareAttack('player', declare58[0].attacker, declare58[0].target);

const passer58 = GameState.combat.defender; // Health 2

const packInst58 = makeCardInstance(fillerChar, 'player'); // Carla Grimsson, Rage 3
GameState.player.pack.push(packInst58);
const tellingBlowCard58 = makeCardInstance(tellingBlow, 'player'); // Rage 3 / Damage 1
GameState.combat.attackerParticipants.push({
  inst: packInst58, ownerWho: 'player', role: 'packmember',
  card: tellingBlowCard58, feintCard: null, feintDecided: false, targetInst: null,
});

const gb58 = getCombatRoundActions('player')
  .find(a => a.type === 'PLAY_COMBAT_CARD' && a.card.name === 'Glancing Blow').card; // Allamande's card, Damage 1
performCombatAction('player', { type: 'PLAY_COMBAT_CARD', card: gb58 });
performCombatAction('opponent', { type: 'PASS_COMBAT_CARD' });
performCombatAction('player', { type: 'CONTINUE_REVEAL' });

// Neither card alone is lethal (Health 2, each does 1) — together they
// are, so whichever one is chosen to resolve SECOND is the one that
// actually crosses the threshold and gets credited with the kill.
check('paused for a real order choice — neither card alone would kill Passer, together they do',
  GameState.combat.step === 'damageOrder');

const orderOpts58 = getCombatRoundActions('player');
check('offered both contested cards to choose an order for',
  orderOpts58.length === 2 && orderOpts58.some(a => a.play.card === gb58) && orderOpts58.some(a => a.play.card === tellingBlowCard58));

// Deliberately resolve Glancing Blow FIRST, Telling Blow SECOND — so
// Telling Blow is the one that crosses the threshold and should be
// credited with the kill (and its bonus VP).
performCombatAction('player', orderOpts58.find(a => a.play.card === gb58));
performCombatAction('player', orderOpts58.find(a => a.play.card === tellingBlowCard58));

check('Passer died and landed in the Victory Pile', GameState.player.victoryPile.some(c => c.instanceId === passer58.instanceId));
check("Telling Blow's bonus VP fired — it was chosen to be the SPECIFIC card that crossed the threshold",
  GameState.player.victoryPile.some(c => c.instanceId === tellingBlowCard58.instanceId && c.vpOverride === 3));

// ═══ TEST 59: Phase 4 — Mindspeak (Gift-granted standing pack-join link) ═══
console.log('\n[59] Phase 4: Mindspeak — links 2 pack members outside combat, they can join together later');

const mindspeak = byName('Mindspeak');
const annaEyes = byName('Anna-Eyes of the Sun-Pelfrey'); // Galliard, Gnosis 7

initGame(
  { characters: [annaEyes, fillerChar], sept: [], combat: [glancingBlow, glancingBlow, glancingBlow, glancingBlow, glancingBlow, glancingBlow] },
  { characters: [tracerPlayerChar], sept: [], combat: [glancingBlow] },
  20
);

GameState.phase = 'resource';
const msInst59 = makeCardInstance(mindspeak, 'player');
GameState.player.septHand.push(msInst59);

const anna59  = GameState.player.pack.find(c => c.name === 'Anna-Eyes of the Sun-Pelfrey');
const carla59 = GameState.player.pack.find(c => c.name === 'Carla Grimsson');

const linkActions59 = getLegalActions('player').filter(a => a.type === 'PLAY_GIFT_PACK_LINK' && a.card.instanceId === msInst59.instanceId);
check('offered a link action for the valid (Galliard/Gnosis-eligible user, packmate) pair',
  linkActions59.some(a => a.user === anna59 && a.mate === carla59));
check('a non-Galliard-eligible pairing is correctly NOT offered (Carla, a Philodox, as user)',
  !linkActions59.some(a => a.user === carla59));

performAction('player', linkActions59.find(a => a.user === anna59 && a.mate === carla59));

check('Mindspeak discarded immediately — the link IS its effect',
  GameState.player.septDiscard.some(c => c.instanceId === msInst59.instanceId));
check('link recorded on player state',
  GameState.player.packLinks.length === 1
  && GameState.player.packLinks[0].a === anna59 && GameState.player.packLinks[0].b === carla59);

// Anna alone declares the attack — Carla isn't in combat yet.
enterPhase('combat');
performAction('player', { type: 'SELECT_ALPHA', card: anna59 });
const declare59 = getLegalActions('player').filter(a => a.type === 'DECLARE_ATTACK');
declareAttack('player', declare59[0].attacker, declare59[0].target);

check('cascades to a general pack-action step — Mindspeak has no step restriction of its own, unlike the declaration-only cards',
  GameState.combat.step === 'preCombat' || GameState.combat.step === 'beginCombat');

const linkJoinOpts59 = getCombatRoundActions('player').filter(a => a.type === 'JOIN_LINKED_PACKMATE');
check('offered to bring in Carla Grimsson via the Mindspeak link', linkJoinOpts59.some(a => a.partner === carla59));

const handSizeBeforeJoin59 = GameState.player.combatHand.length;
performCombatAction('player', linkJoinOpts59.find(a => a.partner === carla59));

check('Carla Grimsson joined via the link', GameState.combat.attackerParticipants.some(p => p.inst === carla59 && p.role === 'packmember'));
check("1 bonus combat card drawn — 'if these characters enter combat'",
  GameState.player.combatHand.length === handSizeBeforeJoin59 + 1);
check('the link was consumed — spent once used, not reusable', GameState.player.packLinks.length === 0);
