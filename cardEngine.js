...
//
// Four trigger kinds, each with exactly one integration point:
//
...
//
//   onDamageDealt — combat.js's applyDamage() calls onDamageResolved()
//                 the instant a Combat Action becomes a damage card,
//                 BEFORE the post-damage Crinos-flip check runs. This
//                 ordering matters: it lets a condition (e.g. Fur
//                 Gnarl's "victim is in Crinos form") read the victim's
//                 form as it was WHEN THE WOUND LANDED, not after a
//                 flip that same wound might itself have just caused.
//                 Takes both the attacker and the victim, so a scripted
//                 effect can target either one (e.g. Off-balanced
//                 Attack debuffs its own player, not the victim).
//
// One more marker, not really a fifth trigger kind since nothing here
// ever executes it — dodgesRound just tags a card as granting a dodge
// (Dodge, Evasion, Evade and Strike). combat.js's resolveBluffAndDamage()
// checks for it directly via cardHasTriggerType() before either side's
// damage is applied, alongside hasKeyword(card,'Undodgeable') from
// game.js for cards like Careful Strike. Kept as a plain trigger-type
// tag rather than inventing a whole new ability shape, since "does this
// card have a dodgesRound ability" is all combat.js ever needs to ask.
//
// feintOnReveal (Feint) works the same way — a tag, not an executed
// effect. combat.js's canFeint() checks for it directly to decide
// whether a side gets an extra card-selection mini-step (its own new
// 'reveal-feint' combat step) after seeing the opponent's revealed
// card, per rule 6.8.1.
//
// endsCombatAfterRound (Run Like Hell, Forceful Wind) is a third such
// marker — checked directly in resolveBluffAndDamage() after this
// round's tiered resolution completes, forcing combat to end right
// there instead of proceeding to the normal withdrawal step. Not
// routed through onDamageDealt: Run Like Hell has no Damage value, so
// applyDamage() (and therefore onDamageResolved()) would never fire
// for it at all.
//
// cancelsOpponentDamageRound1 (Surprise Attack) is a fourth — checked
// after the tiered loop, alongside endsCombatAfterRound, but only in
// round 1. If the tagged card's own damage actually connected (proven
// by presence in the target's damageCards, not just "was it played"),
// the opponent's own damage this round is retroactively removed via
// game.js's removeDamageCard(), same helper regenerate() uses.
//
// awardsBonusVPOnKill (Telling Blow) is a fifth — checked in
// checkCombatDeaths()/awardKillingBlowBonus(), against the SPECIFIC
// card tracked as a creature's killing blow during the tiered
// resolution loop (killingBlowFor), not just any card the killer
// played this round. Redirects that card into the killer's Victory
// Pile with a fixed vpOverride instead of the normal combat discard.
//
...
import {
  ...
  markLastDamageAggravated, applyTempStatMod, applyTempFlag,
} from './game.js';
...
// Lets combat.js ask "does this played card have a scripted ability of
// this trigger type" (e.g. dodgesRound) without needing to know how
// abilities are stored or looked up. `card` may be the string 'pass',
// which trivially has no abilities.
function cardHasTriggerType(card, triggerType) {
  if (!card || card === 'pass') return false;
  return abilitiesFor(card.name).some(ab => ab.trigger?.type === triggerType);
}

// Like cardHasTriggerType, but returns the full trigger object instead
// of a boolean — needed for triggers that carry their own parameters
// (e.g. packJoin's step/side/mode/renownCap/duration) rather than just
// marking that an ability exists. Returns undefined if no matching
// ability is found.
function getTrigger(card, triggerType) {
  if (!card || card === 'pass') return undefined;
  const ab = abilitiesFor(card.name).find(a => a.trigger?.type === triggerType);
  return ab?.trigger;
}
...
// ── onDamageDealt: fired the instant a Combat Action resolves as a
// damage card on its target — see combat.js's applyDamage(), which
// calls this BEFORE the post-damage Crinos-flip check (see header
// comment). `cardInstance` is the Combat Action that just dealt the
// damage; abilities are looked up by ITS name (e.g. "Fur Gnarl").
// `source` is whoever played it (the attacker) and `target` is the
// victim — both are passed through so a scripted effect can apply to
// either one (see the tempStatMod "who" field below).
function onDamageResolved(cardInstance, source, target) {
  for (const ab of abilitiesFor(cardInstance.name)) {
    if (ab.trigger?.type !== 'onDamageDealt') continue;
    if (ab.condition && !evaluateCondition(ab.condition, { inst: target })) continue;
    if (ab.effect?.type === 'markAggravated') {
      markLastDamageAggravated(target);
    }
    if (ab.effect?.type === 'tempStatMod') {
      const who = ab.effect.who === 'self' ? source : target;
      applyTempStatMod(who, {
        stat: ab.effect.stat, mode: ab.effect.mode,
        amount: ab.effect.amount, expiry: ab.effect.expiry,
      });
    }
    if (ab.effect?.type === 'tempFlag') {
      const who = ab.effect.who === 'self' ? source : target;
      applyTempFlag(who, { flag: ab.effect.flag, expiry: ab.effect.expiry });
    }
    EventLog.record('CARD_EFFECT', { who: target.owner, ability: ab.id, card: cardInstance.name });
  }
}
...
function evaluateCondition(cond, ctx) {
  ...
  // Same check as holderIsCrinos, kept as a separate name for
  // onDamageDealt abilities: ctx.inst here is the victim of the wound,
  // not the "holder" of the ability being evaluated.
  if (cond.type === 'targetIsCrinos') {
    return !!ctx.inst?.isCrinos;
  }
...
export {
  ...
  onCardEnteredPlay, onDamageResolved, cardHasTriggerType, getTrigger,
  getStaticModifier,
