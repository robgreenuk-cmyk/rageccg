...
import {
  ...
  onMootResolved, checkPlayCondition, getTrigger,
} from './cardEngine.js';
...
function enterRedraw() {
  ...
  // Mindspeak-style links (rule: "for the remainder of the turn")
  // expire here, at the start of a new one.
  GameState.player.packLinks   = [];
  GameState.opponent.packLinks = [];
...
// Mindspeak-style Gift: resolves instantly (doesn't attach to anyone)
// and links two pack members for the rest of the turn, per rule
// 6.5.8's framing of a card-granted ability to "join in pack attack or
// defence" — checked later in combat.js's eligiblePackLinks() whenever
// a combat is declared for either linked creature. "Discard this Gift
// after its effect takes place" — the linking IS the effect, so it
// discards immediately here; the conditional bonus card ("if these
// characters enter combat") is tracked on the link record itself and
// only drawn later, if/when they actually join combat together.
function doPlayGiftPackLink(who, cardInstance, user, mate) {
  const ps = getPlayer(who);
  removeFrom(ps.septHand, cardInstance);
  cardInstance.zone = 'septDiscard';
  ps.septDiscard.push(cardInstance);

  const trig = getTrigger(cardInstance, 'packLink');
  ps.packLinks = ps.packLinks || [];
  ps.packLinks.push({ a: user, b: mate, bonusCards: trig.bonusCards || 0, grantedBy: cardInstance.name });

  EventLog.record('PACK_LINK', { who, card: cardInstance.name, a: user.name, b: mate.name });
}
...
function enterCombat() {
  ...
  GameState.opponent.alphaActedThisCombatPhase = false;
    ...
    case 'resource':
      ps.septHand.forEach(card => {
        ...
        if (t.startsWith('ally') || t.startsWith('caern') ||
          ...
          const linkTrigger = getTrigger(card, 'packLink');
          if (linkTrigger) {
            // Mindspeak-style: doesn't attach to anyone — resolves
            // instantly (rule 4.5.3's normal "any time" Gift timing,
            // not step-restricted like the Combat Event pack-join
            // cards) and links two DIFFERENT pack members for the
            // rest of the turn. The "user" must meet the card's own
            // Requires keyword and Gnosis requirement; "the packmate"
            // can be anyone else in the pack. Generated as one action
            // per valid (user, packmate) pair — packs are small enough
            // that this doesn't need a two-step picker.
            const reqKeyword = (card.def.Requires || '').trim();
            const gnosisReq = num(card.def.Gnosis);
            const users = ps.pack.filter(m =>
              (!reqKeyword || (m.def.Keywords || '').includes(reqKeyword))
              && effectiveGnosis(m) >= gnosisReq);
            for (const user of users) {
              for (const mate of ps.pack) {
                if (mate === user) continue;
                actions.push({
                  type: 'PLAY_GIFT_PACK_LINK', card, user, mate,
                  label: `Play ${card.name}: link ${user.name} with ${mate.name}`,
                });
              }
            }
            return; // packLink gifts skip the normal teach-to-target path below
          }
...
function getCombatActions(who) {
  ...
  // Rule 2.2.6: "As its alpha action, an alpha may do ONE of the
  // following" (attack, challenge, engage a Battlefield, use an
  // Alpha: ability, or pass). Once that single action has been taken
  // this Combat Phase, there's nothing left to do until next phase —
  // this is what stops a declare-attack/withdraw/re-attack loop.
  if (ps.alphaActedThisCombatPhase) {
    return [{ type: 'ADVANCE_PHASE', label: 'Your alpha has acted — end Combat Phase' }];
  }
  ...
  // Organ Puncture: "cannot initiate combat until this wound is
  // healed. He can, however, defend if attacked" — the "defend"
  // half needs no extra check, since being attacked is always the
  // OTHER side's declareAttack() call, unaffected by this creature's
  // own restriction. This only ever gates THIS alpha's own attack
  // options below.
  if (!hasActiveFlag(ps.alpha, 'cannotInitiateCombat')) {
    if (opp.alpha) {
      actions.push({
...
function performAction(who, action) {
  switch (action.type) {
    ...
    case 'PLAY_GIFT':
      doPlayResource(who, action.card, action.target);
      break;
    case 'PLAY_GIFT_PACK_LINK':
      doPlayGiftPackLink(who, action.card, action.user, action.mate);
      break;
    case 'PLAY_PAST_LIFE':
      doPlayResource(who, action.card, action.target);
      break;
    ...
    case 'SELECT_ALPHA':
      ...
      break;
    case 'PASS_ALPHA':
      getPlayer(who).alphaActedThisCombatPhase = true;
